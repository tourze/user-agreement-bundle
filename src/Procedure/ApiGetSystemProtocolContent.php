...
final class ApiGetSystemProtocolContent extends BaseProcedure
{
    ...
    /**
     * @phpstan-param ApiGetSystemProtocolContentParam $param
     */
    public function execute(ApiGetSystemProtocolContentParam|RpcParamInterface $param): ArrayResult
    {
        if (null === $this->security->getUser()) {
            throw new ApiException('未登录用户不需要同意协议');
        }

        $type = ProtocolType::tryFrom($param->type);
        if (null === $type) {
